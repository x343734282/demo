...
public class WebViewDelegate : WKUIDelegate
{
    ...
    public WebViewDelegate(IWebViewHandler handler)
    {
        ...
        Action<UIContextMenuConfiguration> completionHandler)
    {
        ...
        _handler.TryGetTarget(out IWebViewHandler handler);
        // if (handler != null)
        // {
        //     (((IElementHandler)handler)?.VirtualView as IContextFlyoutElement)?.ContextFlyout?.Handler
        //         ?.DisconnectHandler();
        // }

        foreach (var interaction in webView.Interactions)
        {
            if (interaction is MyContextMenuInteraction cmi)
            {
        ...
        Action<UIAlertController>? cancelAction = null)
    {
        ...
        var handler = (webView.UIDelegate as WebViewDelegate)?.Handler;
